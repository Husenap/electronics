...
static int bcm2835_has_capability(cap_value_t capability) {
  ...
  if (cap) {
    ...
    if (cap_get_flag(cap, capability, CAP_EFFECTIVE, &value) == 0 && value == CAP_SET) ok = 1;
    cap_free(cap);
  }
  return ok;
}
...
void bcm2835_gpio_pudclk(uint8_t pin, uint8_t on) {
  if (pud_type_rpi4) {
    if (on) bcm2835_gpio_set_pud(pin, pud_compat_setting);
  } else {
...
int bcm2835_spi_begin(void) {
  ...
  if (bcm2835_spi0 == MAP_FAILED) return 0; /* bcm2835_init() failed, or not root */
...
void bcm2835_spi_writenb(const char* tbuf, uint32_t len) {
  ...
  */
  ...
  for (i = 0; i < len; i++) {
    ...
    while (bcm2835_peri_read(paddr) & BCM2835_SPI0_CS_RXD) (void)bcm2835_peri_read_nb(fifo);
  }
...
uint8_t bcm2835_i2c_write_read_rs(char* cmds, uint32_t cmds_len, char* buf, uint32_t buf_len) {
  ...
  while (!(bcm2835_peri_read(status) & BCM2835_BSC_S_TA)) {
    /* Linux may cause us to miss entire transfer stage */
    if (bcm2835_peri_read_nb(status) & BCM2835_BSC_S_DONE) break;
  }
...
int bcm2835_smi_begin(void) {
  ...
  if (bcm2835_smi == MAP_FAILED) return 0; /* bcm2835_smi_init() failed, or not root */
  ...
  defConfig = BCM2835_SMI_RW_WID8 | BCM2835_SMI_RW_MODE80 | BCM2835_SMI_RW_PACEALL |
              (10 << BCM2835_SMI_RW_SETUP_LS) | (20 << BCM2835_SMI_RW_STROBE_LS) |
              (20 << BCM2835_SMI_RW_HOLD_LS) | (1 << BCM2835_SMI_RW_PACE_LS);
...
void bcm2835_smi_set_timing(uint32_t smichannel,
  ...
  RegValue &= ~(BCM2835_SMI_RW_SETUP_MSK | BCM2835_SMI_RW_HOLD_MSK | BCM2835_SMI_RW_PACE_MSK |
                BCM2835_SMI_RW_STROBE_MSK);
  /* Set timing values and write back to register */
  RegValue |= (setupcycles << BCM2835_SMI_RW_SETUP_LS) |
              (strobecycles << BCM2835_SMI_RW_STROBE_LS) | (holdcycles << BCM2835_SMI_RW_HOLD_LS) |
...
void bcm2835_smi_write(uint32_t smichannel, uint8_t data, uint32_t address) {
  ...
  bcm2835_peri_write(
      psmiaddr, (smichannel << BCM2835_SMI_DIRADRS_DEV_LS) | (address & BCM2835_SMI_DIRADRS_MSK));
  bcm2835_peri_write(psmidata, data);
  bcm2835_peri_write(psmics,
                     BCM2835_SMI_DIRCS_WRITE | BCM2835_SMI_DIRCS_START | BCM2835_SMI_DIRCS_ENABLE);
...
void bcm2835_pwm_set_clock(uint32_t divisor) {
  ...
  while ((bcm2835_peri_read(bcm2835_clk + BCM2835_PWMCLK_CNTL) & 0x80) != 0) bcm2835_delay(1);
...
void bcm2835_pwm_set_mode(uint8_t channel, uint8_t markspace, uint8_t enabled) {
  ...
  /*  bcm2835_peri_write_nb(bcm2835_pwm + BCM2835_PWM_CONTROL, BCM2835_PWM0_ENABLE |
   * BCM2835_PWM1_ENABLE | BCM2835_PWM0_MS_MODE | BCM2835_PWM1_MS_MODE); */
}
...
int bcm2835_init(void) {
  ...
  */
  ...
  if (geteuid() == 0
    ...
    bcm2835_peripherals =
        (uint32_t*)mapmem("gpio", bcm2835_peripherals_size, memfd, bcm2835_peripherals_base);
