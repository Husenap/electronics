...
/*! \mainpage C library for Broadcom BCM 2835 as used in Raspberry Pi
 ...
 /etc/udev/rules.d/98-mem.rules \endcode \code sudo reboot \endcode
  ...
  from reading /proc/device-tree/soc/ranges. This is only availble with recent versions of the
 kernel on RPI 2.
  ...
  For this library to work correctly on RPI2, you MUST have the device tree support enabled in the
 kernel. You should also ensure you are using the latest version of Linux. The library has been
 tested on RPI2 with 2015-02-16-raspbian-wheezy and ArchLinuxARM-rpi-2 as of 2015-03-29.

  When device tree suport is enabled, the file /proc/device-tree/soc/ranges will appear in the file
 system, and the bcm2835 module relies on its presence to correctly run on RPI2 (it is optional for
 RPI1). Without device tree support enabled and the presence of this file, it will not work on RPI2.
 ...
 bcm2835_spi_setClockDivider()) you should not expect to actually achieve those sorts of speeds with
 the RPi wiring. Our tests on RPi 2 show that the SPI CLK line when unloaded has a resonant
 frequency of about 40MHz, and when loaded, the MOSI and MISO lines ring at an even lower frequency.
 Measurements show that SPI waveforms are very poor and unusable at 62 and 125MHz. Dont expect any
 speed faster than 31MHz to work reliably.
  ...
  allowing you to send and received data by I2C ("eye-squared cee"; generically referred to as
 "two-wire interface") . For more information about I?C, see http://en.wikipedia.org/wiki/I%C2%B2C
  ...
  The BCM2835 contains 2 independent PWM channels (0 and 1), each of which be connnected to a
 limited subset of GPIO pins. The following GPIO pins may be connected to the following PWM channels
 (from section 9.5): \code GPIO PIN    RPi pin  PWM Channel    ALT FUN 12                    0 0 13
 1            0 18         1-12       0            5 19                    1            5 40 0 0 41
 1            0 45                    1            0 52                    0            1 53 1 1
  \endcode
  In order for a GPIO pin to emit output from its PWM channel, it must be set to the Alt Function
 given above. Note carefully that current versions of the Raspberry Pi only expose one of these pins
 (GPIO 18 = RPi Pin 1-12) on the IO headers, and therefore this is the only IO pin on the RPi that
 can be used for PWM. Further it must be set to ALT FUN 5 to get PWM output.
  ...
  Use bcm2835_pwm_set_range() to set the range and bcm2835_pwm_set_data() to set the data in that
 ratio
 ...
 BCM2835_PWM_CLOCK_DIVIDER_* values of \ref bcm2835PWMClockDivider.
  ...
  Since bcm2835 accesses the lowest level hardware interfaces (in eh intererests of speed and
 flexibility) there can be intercations with other low level software trying to do similar things.
  ...
  subject to paging and swapping by the kernel while it does other things besides running your
 program. This means that you should not expect to get real-time performance or real-time timing
 constraints from such programs. In particular, there is no guarantee that the bcm2835_delay() and
 bcm2835_delayMicroseconds() will return after exactly the time requested. In fact, depending on
 other activity on the host, IO etc, you might get significantly longer delay times than the one you
 asked for. So please dont expect to get exactly the time delay you request.
  ...
  utility, spincl, is licensed under Open Source GNU GPLv3 by iP Solutions
 ...
 application. To purchase a commercial license, contact info@airspayce.com
  ...
  \version 1.14 Now compiles even if CLOCK_MONOTONIC_RAW is not available, uses CLOCK_MONOTONIC
 instead. Fixed errors in documentation of SPI divider frequencies based on 250MHz clock. Reported
 by Ben Simpson.
  ...
  \version 1.20 Patch from Mark Dootson to close /dev/mem after access to the peripherals has been
 granted.
  ...
  \version 1.28 Fixed a problem where bcm2835_spi_writenb() would have problems with transfers of
 more than 64 bytes dues to read buffer filling. Patched by Peter Würtz.
  ...
  \version 1.33 Added command line utilities i2c and gpio to examples. Contributed by Shahrooz
 Shahparnia.
  ...
  \version 1.35 Fix build errors when compiled under Qt. Also performance improvements with SPI
 transfers. Contributed b Udo Klaas.

  \version 1.36 Make automake's test runner detect that we're skipping tests when not root, the
 second one makes us skip the test when using fakeroot (as used when building Debian packages).
 Contributed by Guido Günther.
  ...
  Fixed a problem where calling bcm2835_delayMicroseconds loops forever when debug is set. Reported
 by tlhackque.<br> Reinstated use of volatile in 2 functions where there was a danger of lost reads
 or writes. Reported by tlhackque.<br>

  \version 1.41 Added BCM2835_VERSION macro and new function bcm2835_version(); Requested by
 tlhackque.<br> Improvements to peripheral memory barriers as suggested by tlhackque.<br> Reinstated
 some necessary volatile declarations as requested by tlhackque.<br>
 ...
 tlhackque.<br>
  ...
  Improvements to detection of availability of DMB instruction based on value of __ARM_ARCH
 macro.<br>
 ...
 bcm2835_i2c_read_register_rs and others.
 ...
 bcm2835_gpio_set_eds_multi().
  ...
  Fixed a problem where seg fault could occur in bcm2835_delayMicroseconds() if not running as root.
 Patch from Pok.
  ...
  Supports bcm2835_spi_setBitOrder(BCM2835_SPI_BIT_ORDER_LSBFIRST), after which SPI bytes are
 reversed on read or write. Based on a suggestion by Damiano Benedetti.
  ...
  Fixed a bug in bcm2835_i2c_read reported by Charles Hayward where a noisy I2C line cold cause a
 seg fault by reading too many characters.

  \version 1.60 2019-07-23
  Applied patch from Mark Dootson for RPi 4 compatibility. Thanks Mark. Not tested here on RPi4, but
 others report it works. Tested as still working correctly on earlier RPi models. Tested with Debian
 Buster on earlier models
  ...
  being incorrectly converted, this way is clearer and fixes the problem. Contributed by Jonathan
 Perkin.
  ...
  Fixed error in definitions of BCM2835_AUX_SPI_STAT_TX_LVL and BCM2835_AUX_SPI_STAT_RX_LVL. Patch
 from Eric Marzec. Thanks.

  \version 1.65, 1.66 2020-04-16
  Added support for use of capability  cap_sys_rawio to determine if access to /dev/mem is available
 for non-root users. Contributed by Doug McFadyen.

  \version 1.67, 1.66 2020-06-11
  Fixed an error in bcm2835_i2c_read() where the status byte was not correctly updated with
 BCM2835_BSC_S_DONE Reported by Zihan. Thanks.
...
/* RPi 2 is ARM v7, and has DMB instruction for memory barriers.
   ...
   However, not all versions of gcc in all distros support the dmb assembler instruction even on
   compatible processors. This test is so any ARMv7 or higher processors with suitable GCC will use
   DMB.
*/
#if __ARM_ARCH >= 7
...
/*! Physical addresses for various peripheral register sets
  ...
  /proc/device-tree/soc/ranges If /proc/device-tree/soc/ranges exists on a RPi 1 OS, it would be
...
/*! \brief GPIO Pin Numbers
  ...
  If you are using the RPi Compute Module, just use the GPIO number: there is no need to use one of
...
/*! \brief bcm2835SPIClockDivider
  ...
  The system clock frequency on RPi3 is different, so the frequency you get from a given divider
  will be different. See comments in 'SPI Pins' for information about reliable SPI speeds. Note: it
  is possible to change the core clock rate of the RPi 3 back to 250MHz, by putting \code
  core_freq=250
  \endcode
  in the config.txt
*/
...
/* Defines for ST
   ...
   counter. BCM2835_ST_CLO is the System Timer Counter Lower bits register. The system timer
   free-running counter lower register is a read-only register that returns the current value of the
   lower 32-bits of the free running counter. BCM2835_ST_CHI is the System Timer Counter Upper bits
   register. The system timer free-running counter upper register is a read-only register that
   returns the current value of the upper 32-bits of the free running counter.
*/
...
/*! Sets the first 32 GPIO output pins specified in the mask to the value given by value
  ...
  RPI_GPIO_P1_05) \param[in] mask Mask of pins to affect. Use eg: (1 << RPI_GPIO_P1_03) | (1 <<
  RPI_GPIO_P1_05)
*/
...
/*! Sets the SPI bit order
  ...
  BCM2835_SPI_BIT_ORDER_MSB, so if you select BCM2835_SPI_BIT_ORDER_LSB, the bytes will be reversed
  in software. The library defaults to BCM2835_SPI_BIT_ORDER_MSB. \param[in] order The desired bit
  order, one of BCM2835_SPI_BIT_ORDER_*, see \ref bcm2835SPIBitOrder
*/
...
*/
extern void bcm2835_smi_set_timing(uint32_t smichannel,
